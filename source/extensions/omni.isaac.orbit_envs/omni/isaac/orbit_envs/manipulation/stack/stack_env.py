...
from .stack_cfg import StackEnvCfg, RandomizationCfg
from pxr import Gf, PhysxSchema, UsdPhysics, UsdShade, Sdf
import omni


class StackEnv(IsaacEnv):
    """Environment for lifting an object off a table with a single-arm manipulator."""

    def __init__(self, cfg: StackEnvCfg = None, headless: bool = False, enable_camera=False, viewport=False, enable_render=False):
        # copy configuration
        self.cfg = cfg
        ...
        self.target_object = RigidObject(cfg=self.cfg.target_object)
        ...
        self._observation_manager = StackObservationManager(class_to_dict(self.cfg.observations), self, self.device)
        # prepare the reward manager
        self._reward_manager = StackRewardManager(
    ...
    def _design_scene(self) -> List[str]:
        ...
        self.target_object.spawn(self.template_env_ns + "/TargetObject")
    ...
    def _initialize_views(self) -> None:
        ...
        self.target_object_des_pose_w = torch.zeros((self.num_envs, 7), device=self.device)
        # buffers
        self.object_root_pose_ee = torch.zeros((self.num_envs, 7), device=self.device)
        self.target_object_root_pose_ee = torch.zeros((self.num_envs, 7), device=self.device)
        # time-step = 0
        self.object_init_pose_w = torch.zeros((self.num_envs, 7), device=self.device)
        self.target_object_init_pose_w = torch.zeros((self.num_envs, 7), device=self.device)
    ...
    def _randomize_object_desired_pose(self, env_ids: torch.Tensor, cfg: RandomizationCfg.ObjectDesiredPoseCfg):
        """Randomize the initial pose of the object."""
        # get the default root state
        root_state = self.target_object.get_default_root_state(env_ids)
        # -- object root position
        if cfg.position_cat == "default":
            pass
        elif cfg.position_cat == "uniform":
        ...
        root_state[:, 0:3] += self.envs_positions[env_ids]
        # update object init pose
        self.target_object_init_pose_w[env_ids] = root_state[:, 0:7]
        # set the root state
        self.target_object.set_root_state(root_state, env_ids=env_ids)
...
class StackObservationManager(ObservationManager):
    """Reward manager for single-arm reaching environment."""

    def arm_dof_pos(self, env: StackEnv):
        """DOF positions for the arm."""
        return env.robot.data.arm_dof_pos

    def arm_dof_pos_scaled(self, env: StackEnv):
    ...
    def arm_dof_vel(self, env: StackEnv):
        """DOF velocity of the arm."""
        return env.robot.data.arm_dof_vel

    def tool_dof_pos_scaled(self, env: StackEnv):
    ...
    def object_positions(self, env: StackEnv):
        """Current object position."""
        return env.object.data.root_pos_w - env.envs_positions

    def object_orientations(self, env: StackEnv):
...
class StackRewardManager(RewardManager):
    ...
    def reaching_object_position_negative(self, env: StackEnv):
        """Penalize end-effector tracking position error using L2-kernel."""
        error = torch.sum(torch.square(env.robot.data.ee_state_w[:, 0:3] - env.object.data.root_pos_w), dim=1)
        return -error

    def reaching_object_position_exp(self, env: StackEnv, sigma: float):
        """Penalize end-effector tracking position error using exp-kernel."""
        error = torch.sum(torch.square(env.robot.data.ee_state_w[:, 0:3] - env.object.data.root_pos_w), dim=1)
        return torch.exp(-error / sigma)

    def reaching_object_position_tanh(self, env: StackEnv, sigma: float):
    ...
    def tracking_object_position_exp(self, env: StackEnv, sigma: float, threshold: float):
    ...
    def stack_success(self, env: StackEnv):
        ...
        tool_pos = env.robot.data.tool_dof_pos
        mask = torch.logical_and(tool_pos.sum(-1) < 0.06, tool_pos.sum(-1) > 0.038)
        close_enough_to_box = dist < 0.035
    ...
    def placing_objects(self, env: StackEnv, sigma: float, aligning_w: float, aligning_sigma: float):
        aligned_dist = torch.norm(env.object.data.root_pos_w[:, :2] - env.target_object.data.root_pos_w[:, :2], dim=1)
        aligned = aligned_dist < 0.03
        desired_pos = env.target_object.data.root_pos_w[:, :3]
        desired_pos[:, -1] += 0.035
        dist_to_target_object = torch.norm(env.object.data.root_pos_w[:, :3] - desired_pos)

        return 2.5 * aligned * (1 - torch.tanh(dist_to_target_object * sigma)) + aligned * aligning_w * (1 - torch.tanh(aligned_dist * aligning_sigma))
